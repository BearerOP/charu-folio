...
// Transform education data - only if not empty
const educationItems: ExperienceItem[] = education.length > 0 ? education.map(edu => {
  const eduAny = edu as any;
  const gradeInfo = eduAny.cgpa || eduAny.percentage || `Score: ${eduAny.gpa}`;
  return {
    id: eduAny.id || "edu-1",
    title: `${eduAny.degree} in ${eduAny.field}`,
    organization: eduAny.institution,
    location: eduAny.location,
    duration: `${eduAny.startDate} – ${eduAny.endDate}`,
    type: "education" as const,
    description: [eduAny.description, gradeInfo],
    achievements: [...(eduAny.achievements || [])],
    skills: [],
    current: false,
    logo: eduAny.logo,
  };
