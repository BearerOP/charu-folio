...
import { ArrowUpRight, Maximize, Play } from "lucide-react";
...
type Card = {
  ...
  githubLink: string;
  liveLink: string;
  videoUrl?: string;   // Video URL for player
...
export const Card = ({
  ...
  const handleVideoHover = async () => {
    if (videoRef.current && card.videoUrl && !isVideoPlaying) {
      try {
  ...
  return (
    <>
      <div
        ...
        onMouseEnter={() => {
          setIsHovered(true);
          if (card.videoUrl) handleVideoHover();
        }}
        onMouseLeave={() => {
          setIsHovered(false);
          if (card.videoUrl) handleVideoHoverEnd();
        }}
      >
        ...
        <motion.div
          ...
          className="rounded-3xl bg-black dark:bg-neutral-900 h-[20rem] w-56 md:h-[32rem] md:w-80 lg:h-[30rem] lg:w-84 xl:w-84 overflow-hidden flex flex-col items-start justify-start relative z-10 transition-opacity duration-300 cursor-pointer"
          onClick={(e) => {
            e.stopPropagation();
            handleCardClick();
          }}
        >
          ...
          <div className="relative z-40 p-8">
            ...
            {((card.buttonUrl && card.buttonUrl !== "") || (card.githubLink && card.githubLink !== "")) && (
          ...
          {card.videoUrl && (
            <>
              <video
                ...
                loop
                playsInline
                className={`absolute inset-0 w-full h-full object-cover rounded-3xl transition-opacity duration-300 ${isHovered || isVideoPlaying ? "opacity-100" : "opacity-0"
                  }`}
                onEnded={() => setIsVideoPlaying(false)}
              />

              {/* Fullscreen button - shows when video is playing */}
              {isVideoPlaying && (
                <button
                  onClick={handleFullscreen}
                  className="absolute top-4 right-4 z-50 p-2 rounded-full bg-black/50 hover:bg-black/70 transition-colors backdrop-blur-sm"
                >
